...
k562_bw <- BigWigFile("./data/G4/cellline/K562_G4_merged.bw")
k562_res <- summary(k562_bw, du_prom_sig, type = "mean")
du_prom_sig$k562_bw_mean <- mcols(unlist(k562_res))$score

hepg2_bw <- BigWigFile("./data/G4/cellline/HepG2_G4_merged.bw")
hepg2_res <- summary(hepg2_bw, du_prom_sig, type = "mean")
du_prom_sig$hepg2_bw_mean <- mcols(unlist(hepg2_res))$score

pdf("./figure/ap/AP_k562_G4_sig.pdf", width = 6, height = 6.5)
boxplot(
  subset(du_prom_sig, K562_G4_proximity == 1)$k562_bw_mean,
  subset(du_prom_sig, K562_G4_proximity == 0)$k562_bw_mean,
  names = c("G4-overlapped APs", "other APs"),
  col = c("#7570b3", "gray80"),
  border = "gray40",
  ylab = "Average K562 G4 signal",
  outline = FALSE,
  notch = TRUE,
  boxwex = 0.6,
  cex.axis = 1.1,
...
pdf("./figure/ap/AP_hepg2_G4_sig.pdf", width = 6, height = 6.5)
boxplot(
  subset(du_prom_sig, HepG2_G4_proximity == 1)$hepg2_bw_mean,
  subset(du_prom_sig, HepG2_G4_proximity == 0)$hepg2_bw_mean,
  names = c("G4-overlapped APs", "other APs"),
  col = c("#8da0cb", "gray80"),
  border = "gray40",
  ylab = "Average HepG2 G4 signal",
  outline = FALSE,
  notch = TRUE,
  boxwex = 0.6,
  cex.axis = 1.1,
...
k562_bw <- BigWigFile("./data/G4/cellline/K562_G4_merged.bw")
k562_res <- summary(k562_bw, du_prom_gr, type = "mean")
du_prom_gr$k562_bw_mean <- mcols(unlist(k562_res))$score

hepg2_bw <- BigWigFile("./data/G4/cellline/HepG2_G4_merged.bw")
hepg2_res <- summary(hepg2_bw, du_prom_gr, type = "mean")
du_prom_gr$hepg2_bw_mean <- mcols(unlist(hepg2_res))$score

du_prom_gr$G4_diff1 <- du_prom_gr$k562_bw_mean - du_prom_gr$hepg2_bw_mean
du_prom_gr$G4_diff2 <- du_prom_gr$hepg2_bw_mean - du_prom_gr$k562_bw_mean
du_prom_gr$is_ap <- as.numeric(abs(mcols(du_prom_gr)$logFC) > 1 & mcols(du_prom_gr)$FDR < 0.05)

pdf("./figure/ap/G4_signal_diff_AP_vs_nonAP_k562.pdf", width = 6, height = 6.5)
boxplot(
  subset(du_prom_gr, K562_G4_proximity == 1 & is_ap == 1)$G4_diff1,
  subset(du_prom_gr, K562_G4_proximity == 1 & is_ap == 0)$G4_diff1,
  names = c("APs", "non-APs"),
  col = c("#66c2a5", "gray85"),
  border = "gray40",
  ylab = expression(""*Delta*" G4 signal"),
  outline = FALSE,
  notch = TRUE,
  boxwex = 0.6,
  cex.axis = 1.1,
...
pdf("./figure/ap/G4_signal_diff_AP_vs_nonAP_hepg2.pdf", width = 6, height = 6.5)
boxplot(
  subset(du_prom_gr, HepG2_G4_proximity == 1 & is_ap == 1)$G4_diff2,
  subset(du_prom_gr, HepG2_G4_proximity == 1 & is_ap == 0)$G4_diff2,
  names = c("APs", "non-APs"),
  col = c("#fc8d62", "gray85"),
  border = "gray40",
  ylab = expression(""*Delta*" G4 signal"),
...
du_prom_sig_df <- as.data.frame(du_prom_sig)

promoter_classified <- du_prom_sig_df %>%
  dplyr::mutate(
    prom_type = dplyr::case_when(
      logFC > 0 & K562_G4_proximity == 0 & HepG2_G4_proximity == 1 ~ "Gain-Activating",
      logFC < 0 & K562_G4_proximity == 1 & HepG2_G4_proximity == 0 ~ "Loss-Repressing",
      logFC < 0 & K562_G4_proximity == 0 & HepG2_G4_proximity == 1 ~ "Gain-Repressing",
      logFC > 0 & K562_G4_proximity == 1 & HepG2_G4_proximity == 0 ~ "Loss-Activating",
      K562_G4_proximity == 0 & HepG2_G4_proximity == 0 ~ "No-G4",
      K562_G4_proximity == 1 & HepG2_G4_proximity == 1 ~ "Shared-G4",
      TRUE ~ "None"
    )
  )

gene_classified <- promoter_classified %>%
  dplyr::group_by(geneID, symbol) %>%
  dplyr::summarise(
    has_Gain_Activating = any(.data$prom_type == "Gain-Activating"),
    has_Loss_Repressing = any(.data$prom_type == "Loss-Repressing"),
    has_Loss_Activating = any(.data$prom_type == "Loss-Activating"),
    has_Gain_Repressing = any(.data$prom_type == "Gain-Repressing"),
    .groups = "drop"
  ) %>%
  dplyr::mutate(
    gene_type = dplyr::case_when(
      # bidirectional types
      has_Gain_Activating & has_Loss_Repressing ~ "Bidirectional (promotes)",
      has_Gain_Repressing & has_Loss_Activating ~ "Bidirectional (represses)",
      # single mechanism types
      (has_Gain_Activating | has_Loss_Repressing) &
        !has_Gain_Repressing & !has_Loss_Activating ~ "G4 promotes",
      (has_Gain_Repressing | has_Loss_Activating) &
        !has_Gain_Activating & !has_Loss_Repressing ~ "G4 represses",
      TRUE ~ "Other"
    )
  )

gene_summary <- gene_classified %>%
  dplyr::filter(gene_type %in% c(
    "G4 promotes",
    "G4 represses",
    "Bidirectional (promotes)",
    "Bidirectional (represses)"
  )) %>%
  dplyr::count(gene_type, name = "n_genes") %>%
  tidyr::complete(
    gene_type = c("G4 promotes", "G4 represses",
                  "Bidirectional (promotes)", "Bidirectional (represses)"),
    fill = list(n_genes = 0)
  ) %>%
...
color_map <- c(
  "G4 promotes"              = "#3CB371",
  "G4 represses"             = "#E64B35",
  "Bidirectional (promotes)" = "#6A51A3",
  "Bidirectional (represses)"= "#9E9AC8"
)

fig <- ggplot(gene_summary,
              aes(x = reorder(gene_type, n_genes), y = n_genes, fill = gene_type)) +
  geom_col(width = 0.7) +
  ...
  expand_limits(y = max(gene_summary$n_genes) * 1.15)

ggsave("./figure/ap/gene_classified_sorted.pdf", fig, width = 8, height = 6)


sig_sum <- du_prom_sig$K562_G4_proximity + du_prom_sig$HepG2_G4_proximity
gr_sum  <- du_prom_gr$K562_G4_proximity  + du_prom_gr$HepG2_G4_proximity

lv <- sort(unique(c(sig_sum, gr_sum)))
sig_tab <- table(factor(sig_sum, levels = lv))
gr_tab  <- table(factor(gr_sum,  levels = lv))

n1 <- sum(sig_tab); n2 <- sum(gr_tab)

tbl <- rbind(ATSS = as.numeric(sig_tab),
             Background = as.numeric(gr_tab))
ftest <- fisher.test(tbl)

cat("Fisher’s exact test p-value:", ftest$p.value, "\n")

res <- data.frame(
  category = lv,
  sig_prop = as.numeric(sig_tab) / n1,
  gr_prop  = as.numeric(gr_tab) / n2
)
res$ratio <- res$sig_prop / res$gr_prop

res$label <- factor(c("No G4 in either cell line",
                      "G4 in one cell line",
                      "G4 in both cell lines"),
                    levels = c("No G4 in either cell line",
                               "G4 in one cell line",
                               "G4 in both cell lines"))

df <- reshape2::melt(res[, c("label","sig_prop","gr_prop")],
                     id.vars="label", variable.name="group", value.name="prop")

p_max  <- max(df$prop)
r_min  <- 0
r_max  <- max(res$ratio)
to_prop   <- function(r) (r - r_min)/(r_max - r_min) * p_max
from_prop <- function(p) (p / p_max) * (r_max - r_min) + r_min

p_g4_dist <- ggplot(df, aes(x=label)) +
  geom_bar(aes(y=prop, fill=group),
           stat="identity", position=position_dodge(0.8), width=0.7,
           color="black", linewidth=0.25) +
  geom_line(data=res, aes(y=to_prop(ratio), group=1),
            color="#C0C0C0", linewidth=1.3) +
  geom_point(data=res, aes(y=to_prop(ratio)),
             color="#C0C0C0", size=3) +
  geom_text(data=res, aes(y=to_prop(ratio),
                          label=sprintf("Ratio = %.2f", ratio)),
            vjust=-0.8, size=4.5, color="#707070", family="Helvetica") +
  scale_fill_manual(
    values=c("sig_prop"="#D9822B", "gr_prop"="#4C8CBF"),
    labels=c("AP", "Background promoters")
  ) +
  scale_y_continuous(
    name = "Proportion of promoters",
    sec.axis = sec_axis(~ from_prop(.),
                        name = "Ratio of proportions (AP / Background)")
  ) +
  labs(
    subtitle = sprintf("Fisher’s exact test, p = %.2e", ftest$p.value)
  ) +
  theme_classic(base_size=16) +
  theme(
    axis.line = element_line(linewidth=0.8, colour="black"),
    axis.ticks = element_line(linewidth=0.8, colour="black"),
    axis.text.x = element_text(angle=20, hjust=1, size=13, colour="black"),
    axis.title.x = element_blank(),
    ...
    plot.subtitle = element_text(size=13, hjust=0.5, colour="#505050"),
    plot.margin = margin(t = 25, r = 10, b = 35, l = 50),
    axis.text = element_text(size=13, color="black")
  ) +
  coord_cartesian(clip = "off")

ggsave("./figure/ap/prop_G4.pdf", p_g4_dist, width = 8.5, height = 6)
